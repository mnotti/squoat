...
class MenuScene: SKScene {
    ...
    func createHighScoreNodes(offset: CGFloat){
        ...
        if (connectedToNetwork()){
            ...
            let couldntLoad = SKLabelNode(fontNamed: "Chalkduster")
            couldntLoad.text = "Couldn't Load High Scores..."
            couldntLoad.fontSize = 15
            couldntLoad.position = CGPointMake(size.width/2 + offset, size.height - 70)
            couldntLoad.name = "scoreNode"
            couldntLoad.zPosition = 1
            self.addChild(couldntLoad)
